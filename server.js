  ...
  try {
    if (!API_KEY) {
      return res.status(500).json({
        ok: false,
        error: "Twelve Data API key belum dipasang di server."
      });
    }
    ...
    if (!intervals[tf]) {
      return res.status(400).json({
        ok: false,
        error: "Timeframe harus M5, M15, H1 atau H4."
      });
    }
    ...
    if (candles.length < 30) {
      return res.status(502).json({
        ok: false,
        error: "Data candle yang diterima belum cukup."
      });
    }
...
Penting:
- Ini bukan nasihat investasi atau keuangan.
- Jangan menjanjikan profit.
- Jangan mengatakan bahwa arah market pasti naik atau turun.
- Jika data tidak cukup jelas, katakan NEUTRAL.
- Selalu jelaskan bahwa trading memiliki risiko kehilangan modal.
`;
...
app.listen(
  PORT,
  () => console.log(`SniperGold V2: http://localhost:${PORT}`)
);